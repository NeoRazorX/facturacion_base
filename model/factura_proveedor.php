...
//   public $idasientofac;
   ...
   public function all_sin_pagar($offset=0, $limit=FS_ITEM_LIMIT, $orden)
   {
      $faclist = array();
      $facturas = $this->db->select_limit("SELECT * FROM ".$this->table_name.
         " WHERE pagada = false AND idpagodevol = '0' and idasiento != 'NULL' ORDER BY ".$orden, $limit, $offset);
      if($facturas)
      {
         foreach($facturas as $f)
            $faclist[] = new factura_proveedor($f);
   ...
   public function all_sin_pagar_com($offset=0, $limit=FS_ITEM_LIMIT)
   {
      $faclist = array();
      $facturas = $this->db->select("SELECT * FROM ".$this->table_name.
         " WHERE pagada = false AND idpagodevol = '0' and idasiento != 'NULL' ORDER BY fecha ASC, codigo ASC");
      if($facturas)
      {
         foreach($facturas as $f)
            $faclist[] = new factura_proveedor($f);
  ...
  //    $faclist = array();
   ...
   public function pagada_true($codigo,$pagado)
   {
   			
       $sql = "UPDATE facturasprov SET pagada = ".$this->var2str($pagado)." WHERE codigo = ".$this->var2str($codigo).";";
           if( $this->db->exec($sql) )
            {
               return TRUE;
            }

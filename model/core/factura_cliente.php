...
class factura_cliente extends \fs_model
{
      ...
      if( is_null($this->numero) ) /// nueva factura
      {
         ...
         if($data)
         {
            if( strtotime($data[0]['fecha']) > strtotime($fecha) )
            {
               ...
               $this->new_error_msg('Ya hay facturas posteriores a la fecha seleccionada.'
   ...
   public function new_codigo()
   {
      ...
      elseif (FS_NEW_CODIGO == 'new') 
      {
        $this->codigo = 'FAC'.$this->codejercicio.$this->codserie.$this->numero;
      }
      elseif (FS_NEW_CODIGO == 'number_year')
      {         
        $this->codigo = $this->codserie.$this->numero.'/'.$this->codejercicio;
      }
   }
   ...
   public function huecos()
   {
      ...
      if($error)
      {
         ...
         foreach($ejercicio->all_abiertos() as $eje)
         {
            $codserie = '';
            $num = 1;
            $sql = "SELECT codserie,".$this->db->sql_to_int('numero')." as numero,fecha,hora
